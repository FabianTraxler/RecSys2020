...
from pyspark.ml.feature import Word2Vec, StandardScaler, QuantileDiscretizer, StringIndexer, FeatureHasher, OneHotEncoderEstimator, CountVectorizer,PCA, VectorAssembler
from pyspark.ml import Pipeline
...
def create_countVectorizer(input_col: str) -> CountVectorizer:
    ...
    FeatureHasher
    """
    output_col = input_col + "_tokens"
    return CountVectorizer(inputCol=input_col,
                           outputCol=output_col)

def create_word2Vec(input_col:str) -> Word2Vec:
    """
    Create a Word2Vec Model for a specified column 
    Uses as output colum the input + _encoded (Count Vectors for every column)
    ...
    """
    output_col = input_col + "_encoded"
    return  Word2Vec(vectorSize=50, minCount=0, 
                        inputCol=input_col, 
                        outputCol=output_col)
...
if __name__ == "__main__":
    ...
    tweet_feature_cols = ['text','hashtags',
                        'present_media', 
                        'present_links', 
                        'present_domains']
